...
import { createLocationCard } from "../location-cards/actions";
import { cldThumb } from "@/lib/cloudinary";

export default function LocationCardForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await createLocationCard(cityName, imageUrl);
    } catch {
      alert("Save failed");
      setSaving(false);
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white border border-sage/20 rounded-lg p-4 mb-6">
      <h3 className="font-display text-lg text-forest mb-3">Add Location Card</h3>

      <label className="block text-sm text-ink/80 mb-1">City Name</label>
      <input value={cityName} onChange={(e) => setCityName(e.target.value)} required className={`${inputClass} mb-4`} />
