...
import { addMoreAbout, deleteMoreAbout } from "../../content-actions";
import DeleteGenericButton from "../../../components/DeleteGenericButton";
import { ICON_OPTIONS } from "@/lib/breederIcons";
import { notFound } from "next/navigation";

export default async function MoreAboutPage({ params }: { params: Promise<{ breederId: string }> }) {
  ...
  const removeItem = async (id: string) => {
    "use server";
    await deleteMoreAbout(id, breeder.slug);
  };
  ...
  return (
    <main className="px-5 pt-6 pb-10">
      ...
      <h1 className="font-display text-xl text-forest mb-6">More About {breeder.name}</h1>
