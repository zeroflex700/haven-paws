...
function cloudinaryLoader({ src, width, quality }: { src: string; width: number; quality?: number }) {
  ...
  return src.replace("/upload/", `/upload/w_${width},q_${quality ?? "auto"},f_auto,c_limit/`);
}

function blurUrl(src: string): string | null {
  if (!src.includes("/upload/")) return null;
  return src.replace("/upload/", "/upload/w_32,e_blur:1000,q_1,f_auto/");
}
...
export default function OptimizedImage({
  ...
  if (!src) {
    return (
      <div className={`w-full h-full bg-cream-alt flex items-center justify-center ${containerClassName}`}>
