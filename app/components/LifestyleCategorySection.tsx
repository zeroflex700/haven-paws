...
export default function LifestyleCategorySection({
  ...
  return (
    <section id={category.key} className={`py-14 scroll-mt-20 ${alt ? "bg-cream-alt" : ""}`}>
      <div className="max-w-2xl mx-auto px-6">
        ...
        <h2 className="font-display text-2xl text-forest mt-4 mb-3">{category.title}</h2>
        <p className="text-sage leading-relaxed whitespace-pre-line mb-6">
          {category.description}
        </p>

        <BreedCarousel breeds={category.breeds} imageMap={imageMap} />
      </div>
    </section>
  );
}